...
@section('admin-title')<h1>pages — список</h1>@endsection
...
<form method="get" class="form-actions" style="margin-bottom:10px">
  ...
  <a href="{ route('admin.pages.create') }" class="btn">Создать</a>
</form>
  ...
  @forelse($items as $it)
    <tr>
      ...
      <td class="admin-actions">
        <a href="{ route('admin.pages.edit', $it) }" class="btn secondary">Редактировать</a>
        <form method="post" action="{ route('admin.pages.destroy', $it) }" onsubmit="return confirm('Удалить?')">
          @csrf @method('DELETE')
