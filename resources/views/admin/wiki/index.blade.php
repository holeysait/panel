...
@section('admin-title')<h1>wiki — список</h1>@endsection
...
<form method="get" class="form-actions" style="margin-bottom:10px">
  ...
  <a href="{ route('admin.wiki.create') }" class="btn">Создать</a>
</form>
  ...
  @forelse($items as $it)
    <tr>
      ...
      <td class="admin-actions">
        <a href="{ route('admin.wiki.edit', $it) }" class="btn secondary">Редактировать</a>
        <form method="post" action="{ route('admin.wiki.destroy', $it) }" onsubmit="return confirm('Удалить?')">
          @csrf @method('DELETE')
